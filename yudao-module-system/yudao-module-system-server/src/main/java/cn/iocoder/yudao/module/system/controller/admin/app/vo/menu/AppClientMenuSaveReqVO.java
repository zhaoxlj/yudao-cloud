...
@Schema(description = "管理后台 - App客户端菜单创建/修改 Request VO")
@Data
public class AppClientMenuSaveReqVO {
    ...
    @Schema(description = "权限标识,仅菜单类型为按钮时，才需要传递", example = "app:menu:add")
    @Size(max = 100)
    ...
    @Schema(description = "路由地址,仅菜单类型为菜单或者目录时，才需要传", example = "app-menu")
    ...
    @Schema(description = "组件路径,仅菜单类型为菜单时，才需要传", example = "app/menu/index")
